...
thread_get_load_avg (void)
{
  /* Not yet implemented. */
  // enum intr_level old_level = intr_disable ();
  int avg = _TO_INT_NEAREST (_MULTIPLY_INT (load_avg, 100));
  // intr_set_level(old_level);
  return avg;
...
thread_get_recent_cpu (void)
{
  /* Not yet implemented. */
  // enum intr_level old_level = intr_disable ();
  int cpu = _TO_INT_NEAREST (_MULTIPLY_INT (thread_current ()->recent_cpu, 100));
  // intr_set_level(old_level);
  return cpu;
}

...
void set_wakeup_next(void)
{
  ...
  else
  {
    ...
    struct thread *th = list_entry(f, struct thread, elem);
      ...
      if(list_empty(&sleepers))		
        ...
        { 
         // next first element in list
          ...
          th = list_entry(f, struct thread, elem); 
  ...
  for (y = list_begin (&all_list); y != list_end (&all_list); y = list_next (y))
    {
      ...
      if( t!=initial_thread && t!=idle_thread && t!=wakeup_managerial_thread && t!=mlfqs_managerial_thread){
        // int temp = ( ) ;
 ...
 void thread_mlfqs_change_priority(void){
  ...
  for (y = list_begin (&all_list); y != list_end (&all_list); y = list_next (y))
  {
    ...
    if(  t!=idle_thread && t!=wakeup_managerial_thread && t!=mlfqs_managerial_thread){
     	...
     	int prio = _TO_INT_ZERO (_INT_SUB (PRI_MAX, aux));
