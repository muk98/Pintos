...
extern struct thread *array1[2040];
extern int array2[2040];
extern int array3[2040];
...
struct thread
  {
    ...
    int pre_priority;                   /*  Previous Priority.*/
    int priority_before;                /* priority before donation */
    struct list_elem allelem;           /* List element for all threads list. */
    int nice;
    int recent_cpu;
    /* Shared between thread.c and synch.c. */
    struct list_elem elem;              /* List element. */
    int64_t wakeup_t;                   /* wakeup time of the thread */
#ifdef USERPROG
    ...
    struct list acquired_locks;         /* The list of locks a thread has currently acquired */
    struct lock *seeking;               /* The lock a thread is seeking to acquire */
    struct file *files[MAX_FILES];
    // struct list_elem parent_elem;       /*list element for parent-child list*/
    // struct thread *parent;
    // struct list children;
    struct semaphore sema_ack;
    struct semaphore thread_load_done;
