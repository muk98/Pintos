...
void thread_run_untill(int64_t wakeup_at, bool *before){
  ...
  if(list_empty(&sleepers))next_wakeup_time=list_entry(list_begin(&sleepers),struct thread,elem)->wakeup_at;
...
void set_wakeup_next(void){
  ...
  if(!list_empty(&sleepers)){
    ...
    struct thread *t2= list_entry(list_begin(&sleepers),struct thread,elem);

    if(t2->wakeup_at == t->wakeup_at){
