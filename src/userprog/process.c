...
start_process (void *file_name_)
{
  ...
  if (!success){ 
    ...
    array2[thread_current()->tid] = 0;
    sema_up(&thread_current()->thread_load_done);
    sema_up(&thread_current()->sema_exit);
    thread_exit ();
  }
  // int status = array3[thread_current()->tid];
  array2[thread_current()->tid] = 1;
  sema_up(&thread_current()->thread_load_done);
