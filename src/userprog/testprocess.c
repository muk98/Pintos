...
test_stack (int *t)
{
  ...
  for (i = 0; i < argc; i++)
    printf("Argv[%d] = %x pointing at %s\n",
           i, (unsigned int)argv[argc-i], argv[argc-i]);
...
setup_stack (void **esp, char *file_name, char *args) 
{
  ...
  /* Obtain a single free page and stores its kernel virtual address to kpage. */
  kpage = palloc_get_page (PAL_USER | PAL_ZERO);

  /* If atleast one page is available */
  if (kpage != NULL) 
    {
      success = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true);
      if (success){

        /* PHYS_BASE = 0xc0000000, end of user programs' address space */
        *esp = PHYS_BASE;

        /* Tokenize the string accross spaces (DELIMITER) */
        char *token, *save_ptr;

        /* argc is initialised by 1 since the file_name arg. is already included */ 
        int argc = 1;
        char *argv[LOADER_ARGS_LEN / 2 + 1];

        argv[0] = file_name;

        /* store each argument as a token separately */
        ...
        /* store last argument as NULL by Default */
        argv[argc] = NULL;
        ...
        /* shift down by 11 memory spaces */
        *esp -= s;

        // addr[0]=(char*)* esp;

        /* Push the arguments to the stack and store their addresses  */
        for (i = 0; i<argc; i++)
        {
        ...
        /* Make the used address space a multiple of 4 
           by pushing extra 0s as needed */
        while((int)*esp%4!=0)
          {
            *esp-=sizeof(char);
            char x = 0;
